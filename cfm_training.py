...
def train_CFM(args):
    ...
    # 只训练 ResNet 解冻部分 + 两个映射网络
    backbone_params = [
        p for _, p in feature_extractor.rgb_backbone.named_parameters() if p.requires_grad
    ]
    ...
    print(f"\n🔥 启动稳定回退版: 56x56 + avg_pool + 冻结 freq_backbone | 类别: {args.class_name}")
        ...
        for (rgb, freq_img), _ in pbar:
            ...
            if args.batch_size == 1:
                ...
                for i in range(rgb.shape[0]):
                    rp, fp = feature_extractor.get_features_maps(
                        rgb[i].unsqueeze(dim=0),
                        freq_img[i].unsqueeze(dim=0)
                    )
    ...
    torch.save(
        CFM_RGBtoFreq.state_dict(),
        os.path.join(directory, f'CFM_RGBtoFreq_{model_name}.pth')
    )
    torch.save(
        CFM_FreqtoRGB.state_dict(),
        os.path.join(directory, f'CFM_FreqtoRGB_{model_name}.pth')
    )
    torch.save(
        feature_extractor.deep_feature_extractor.freq_backbone.state_dict(),
        os.path.join(directory, f'FreqBackbone_{model_name}.pth')
    )
